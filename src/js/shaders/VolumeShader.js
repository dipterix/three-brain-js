...
function RayMarchingShader ({
  modelShape,
  cmap = null,
  colorChannels = 4,
  alpha = -1.0,
  singleChannelPalette
} = {}) {
  if( !Array.isArray(singleChannelPalette) ) {
    singleChannelPalette = [
      new Color().setHex(0x999999),
      new Color().setHex(0x006400),
      new Color().setHex(0xFF4500)
    ]
  }
  const VolumeRenderShader1 = {
    uniforms: {
      cmap: { value: cmap },
      alpha : { value: alpha },
      colorChannels : { value: colorChannels },
      // steps: { value: 300 },
      scale_inv: { value: new Vector3().set( 1 / modelShape.x, 1 / modelShape.y, 1 / modelShape.z ) },
      bounding: { value : 0.5 },
      ...
      // only works when number of color channels is 1
      colorsWhenSingleChannel: { value: singleChannelPalette },
      singleChannelColorRangeLB: { value: 0.0 },
      singleChannelColorRangeUB: { value: 1.0 },
...
#define N_SINGLE_CHANNEL_COLORS ${ singleChannelPalette.length }
precision highp float;
precision mediump sampler3D;
in vec3 vOrigin;
in vec3 vPosition;
